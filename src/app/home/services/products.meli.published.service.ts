...
export class ProductsMeliPublishedService {
  ...
  public getProductsPublished(page: number, size: number, skuSearch: string, idMeliSearch: string, meliAccountSearch: number,
    typeStateSearch: string, title: string, inStock: boolean): Observable<PageProductMeliPublished> {
    let params: string = `page=${page}&size=${size}`;
    skuSearch.length > 0 && (params += `&sku=${skuSearch.trim()}`);
    idMeliSearch.length > 0 && (params += `&idMeliPublication`, idMeliSearch.trim());
    meliAccountSearch && (params += `&meliAccount=${meliAccountSearch.toString()}`);
    typeStateSearch.length > 0 && (params += `&typeStateSearch=${typeStateSearch.trim()}`);
    ...
    return this.http.get<PageProductMeliPublished>(`${this.URI_PRODUCTS_PUBLISHED_SERVICE}/${this.profileId}?${params}`);
  }
